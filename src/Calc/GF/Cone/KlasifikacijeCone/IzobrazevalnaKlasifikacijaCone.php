...
class IzobrazevalnaKlasifikacijaCone extends KlasifikacijaCone
{
    ...
    public function referencniTSSRazsvetljava(Cona $cona): array
    {
        ...
        - faktor zmanjšanja projektne osvetlitve FCA kot pri obravnavani stavbi, upoštevajo se vrednosti iz projektne
        dokumentacije;
        ...
        $ret->letnoUrPodnevi = 1800;
        $ret->letnoUrPonoci = 200;
    ...
    public function referencniTSSPrezracevanja(Cona $cona): array
    {
        ...
        - količine zraka se prevzamejo glede na vrsto cone in urnik zasedenosti stavbe iz standarda SIST ISO 185231;
        ...
        - pogoni SFP 3 dovod (0,211 W/(m3/h)), SFP 2 dovod (0,142 W/(m3/s)), (tabela 14 v standardu SIST EN 167983,
            z upoštevanjem sestavnih komponent, kot so navedene v tabeli 15 v standardu SIST EN 167983);
        - naravno prezračevanje in hibridno prezračevanje se ne upoštevata; brez predogrevanja in predhlajenja zraka.
        */
    ...
    public function referencniTSSOHT(Cona $cona): array
    {
        ...
        */
        ...
        $ogrtsv->tsv->generatorji = ['SSE', 'ELGRELNIK'];
        $ogrtsv->tsv->razvodi = ['TSV'];
        $ogrtsv->tsv->hranilniki = ['TSVH'];

        $generator2 = new \stdClass();
        $generator2->id = 'ELGRELNIK';
        $generator2->vrsta = 'elektricniGrelnik';
        $generator2->nazivnaMoc = 2;
        $generator2->stevilo = (int)round($cona->ogrevanaPovrsina / 100);
        $ogrtsv->generatorji[] = $generator2;
    ...
    public function referencniTSSFotovoltaika(Cona $cona): array
    {
        ...
        $ret->id = $cona->id;
        $ret->idCone = $cona->id;
        $ret->vrsta = 'monokristalne';
        $ret->povrsina = 0.04 * $cona->ogrevanaPovrsina;
        $ret->orientacija = 'J';
        $ret->naklon = 30;
        $ret->sencenje = false;
