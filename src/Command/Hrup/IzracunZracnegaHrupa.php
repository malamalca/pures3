...
class IzracunZracnegaHrupa extends Command
{
    /**
     * Command run routine
     *
    ...
    public function run($projectId = null)
    {
        ...
        if ($hrupIn) {
            ...
            foreach ($hrupIn as $konstrukcijaConfig) {
                $konstrukcija = new ZracniHrupPoenostavljen($konstrukcijeLib, $konstrukcijaConfig);
                $konstrukcija->analiza($splosniPodatki);
                $hrupOut[] = $konstrukcija->export();
            }

            App::saveProjectCalculation('Hrup', $projectId, 'zracniHrup', $hrupOut);
        }
    }
}
